...
（$\Leftarrow$）反之，若 $\langle\boldsymbol{a},\boldsymbol{a}'\rangle=0$，则 $\frac{\mathrm d}{\mathrm dt}\|\boldsymbol{a}\|^{2}=0$，故 **$\|\boldsymbol{a}\|^{2}$ 为常数，$|\boldsymbol{a}|$ 为常数**。$\blacksquare$
...
$$
\boxed{L(\boldsymbol\alpha,[a,b])=\int_a^b|\boldsymbol\alpha'|\mathrm dt}\quad\blacksquare
$$
...
故任何连接两点的曲线长度都不小于 $|Q-P|$，而直线段恰好达到 $|Q-P|$，所以 **$\mathbb R^3$ 中任何两点之间直线最短**。$\blacksquare$
...
综上，平面曲线之特别处在于：曲率退化为一个可带符号的标量函数、且无挠率，由单一函数 $\kappa(s)$ 模保向等距即可完全确定曲线。$\blacksquare$
...
\Rightarrow&&&\kern-0.9em\boxed{\kappa\ge\frac1r}\quad\blacksquare
\end{aligned}
$$
...
综上，满足该条件的曲线，或者**落在球面上**（情形 A），或者 **$\kappa$ 为常数**（情形 B）。$\blacksquare$
...
即 $\langle\boldsymbol r(t_0)-P_0,\boldsymbol r'(t_0)\rangle=0$。故向量 **$\boldsymbol r(t_0)-P_0$ 与 $\boldsymbol r'(t_0)$ 垂直**。$\blacksquare$
...
若某开区间上 $\kappa\ne0$，则 $\boldsymbol T,\boldsymbol N$ 线性无关，比较系数得 $\lambda'=1$ 且 $\lambda\kappa=0$。由 $\kappa\ne0$ 得 $\lambda=0$，进而 $\lambda'=0\ne1$，矛盾。故在整个区间上 $\kappa\equiv0$，即 $\dot{\boldsymbol T}\equiv\boldsymbol 0$。由 **$\dot{\boldsymbol T}\equiv\boldsymbol 0$ 当且仅当曲线是直线段**，曲线 $C$ **是直线**。$\blacksquare$
...
即曲线到原点的距离恒定。落在平面内且到定点距离恒定的曲线是**以原点为圆心、半径 $|\lambda|=\frac1\kappa$ 的圆**。$\blacksquare$
...
$$
\boxed{\tilde s=s,\quad \tilde\kappa=\kappa,\quad \tilde\tau=-\tau}\quad\blacksquare
$$
